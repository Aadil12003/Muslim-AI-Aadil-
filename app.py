...
# Updated Base Prompt to enforce valid JSON, Hinglish, Transliteration, and Systematic scholarly layouts
...
- DO NOT use literal line breaks (pressing Enter) inside the JSON response. You MUST use \\n for line breaks to keep the JSON valid.
...
- SCHOLARLY PRECISION: Separate the 'opinion', 'reasoning' (logic), and 'evidence' (exact verse/hadith) clearly.
...
def parse_response(raw):
    try:
        ...
        # Clean unescaped newlines to prevent JSON decoding crashes (Error 500)
        cleaned = cleaned.replace('\n', '\\n').replace('\r', '')
        
        return normalize_result(json.loads(cleaned, strict=False))
    except Exception: 
        return normalize_result({"direct_answer": str(raw).replace('\n', '<br>')})
...
def render_response(result):
    ...
    # Render Missing Duas (AI Search Bug Fix)
    if result.get("duas"):
...
with tab2: 
    ...
    with col1:
        ...
        audio_type = st.radio("Select Recitation Audio", ["Arabic Only (Mishary Alafasy)", "Urdu Translation Only (Fateh Muhammad Jalundhary)"])
    ...
    with col2:
        if st.session_state.loaded_surah_number:
            # Safe and Direct Archive.org Links for Urdu Translation
            if audio_type == "Arabic Only (Mishary Alafasy)":
                ...
                # Official Archive.org URL for Urdu-only translation (100% reliable)
                audio_url = f"https://archive.org/download/quran_urdu_audio_only/{st.session_state.loaded_surah_number:03d}.mp3"
...
with tab4:
    ...
    for dua in DUA_CATEGORIES.get(selected_category, []):
        st.markdown(
            ...
            f'<div class="arabic" style="margin: 20px 0;">{safe_html(dua.get("arabic", ""))}</div>'
            f'<strong class="accent" style="font-size:14px; text-transform:uppercase;">Transliteration</strong><br><span style="color:#CBD5E1; line-height:1.6; display:inline-block; margin-bottom:16px;">{safe_html(dua.get("transliteration", ""))}</span><br>'
            f'<strong class="accent" style="font-size:14px; text-transform:uppercase;">Meaning (English)</strong><br><span style="font-size:16px; line-height:1.6; color:#F8FAFC;">{safe_html(dua.get("meaning", ""))}</span>'
            f'<div style="margin-top:20px; border-top:1px solid rgba(251, 191, 36, 0.2); padding-top:12px;"><span class="muted">Reference: {source_link(dua.get("reference", ""), dua.get("source_url", ""))}</span></div></div>',
            unsafe_allow_html=True,
        )
